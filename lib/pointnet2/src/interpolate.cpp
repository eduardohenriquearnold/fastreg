...
void nn_ddim_wrapper_fast(int b, int n, int m, int d, at::Tensor unknown_tensor, 
    ...
    nn_ddim_kernel_launcher_fast(b, n, m, d, unknown, known, dist2, idx, stream);
}
...
void nn_ddim_grad_wrapper_fast(int b, int n, int m, int d, at::Tensor unknown_tensor, 
  at::Tensor known_tensor, at::Tensor idx_tensor, at::Tensor grad_dist2_tensor, at::Tensor grad_unknown_tensor, at::Tensor grad_known_tensor) {
    const float *unknown = unknown_tensor.data<float>();
    const float *known = known_tensor.data<float>();
    const float *grad_dist2 = grad_dist2_tensor.data<float>();
    float *grad_unknown = grad_unknown_tensor.data<float>();
    float *grad_known = grad_known_tensor.data<float>();
    const int *idx = idx_tensor.data<int>();

    cudaStream_t stream = at::cuda::getCurrentCUDAStream();
    nn_ddim_grad_kernel_launcher_fast(b, n, m, d, unknown, known, idx, grad_dist2, grad_unknown, grad_known, stream);
}
