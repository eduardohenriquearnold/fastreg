...
void three_nn_kernel_launcher_fast(int b, int n, int m, const float *unknown, 
 ...
 __global__ void nn_ddim_kernel_fast(int b, int n, int m, int d, const float *__restrict__ unknown, 
    const float *__restrict__ known, float *__restrict__ dist2, int *__restrict__ idx) {
    // unknown: (B, N, D)
    // known: (B, M, D)
    // output: 
    ...
    for (int k = 0; k < m; ++k) {
        ...
        for(int di = 0; di < d; ++di){
          float t1 = (unknown[di] - known[k * d + di]);
          dist += t1*t1;
        }
...
void nn_ddim_kernel_launcher_fast(int b, int n, int m, int d, const float *unknown, 
    ...
    nn_ddim_kernel_fast<<<blocks, threads, 0, stream>>>(b, n, m, d, unknown, known, dist2, idx);
 ...
 __global__ void nn_ddim_grad_kernel_fast(int b, int n, int m, int d, const float *__restrict__ unknown, 
    const float *__restrict__ known, const int *__restrict__ idx, const float *__restrict__ grad_dist2, float *__restrict__ grad_unknown, float *__restrict__ grad_known) {
    // unknown: (B, N, D)
    // known: (B, M, D)
    // grad_dist2: (B, N, D)
    // idx : (B,N,3)
    ...
    unknown += bs_idx * n * d + pt_idx * d;
    idx += bs_idx * n * 3 + pt_idx * 3;
    grad_dist2 += bs_idx * n * 3 + pt_idx * 3;

    const float* known0 = known + bs_idx * m * d + idx[0] * d;
    const float* known1 = known + bs_idx * m * d + idx[1] * d;
    const float* known2 = known + bs_idx * m * d + idx[2] * d;

    grad_unknown += bs_idx * n * d + pt_idx * d;
    float* grad_known0 = grad_known + bs_idx * m * d + idx[0] * d;
    float* grad_known1 = grad_known + bs_idx * m * d + idx[1] * d;
    float* grad_known2 = grad_known + bs_idx * m * d + idx[2] * d;

    for (int di=0; di<d; di++)
    {
      float t0 = grad_dist2[0] * 2 * (unknown[di] - known0[di]);
      float t1 = grad_dist2[1] * 2 * (unknown[di] - known1[di]);
      float t2 = grad_dist2[2] * 2 * (unknown[di] - known2[di]);
      grad_unknown[di] += t0 + t1 + t2;
      atomicAdd(grad_known0 + di, -t0); //atomic is required because multiple threads will try to write to the same position simultaneously
      atomicAdd(grad_known1 + di, -t1);
      atomicAdd(grad_known2 + di, -t2);
    }

}


void nn_ddim_grad_kernel_launcher_fast(int b, int n, int m, int d, const float *unknown,
	const float *known, const int *idx, const float *grad_dist2, float *grad_unknown, float *grad_known, cudaStream_t stream) {
    // unknown: (B, N, D)
    // known: (B, M, D)
    // grad_dist2: (B, N, D)
    // idx : (B,N,3)
    ...
    nn_ddim_grad_kernel_fast<<<blocks, threads, 0, stream>>>(b, n, m, d, unknown, known, idx, grad_dist2, grad_unknown, grad_known);
